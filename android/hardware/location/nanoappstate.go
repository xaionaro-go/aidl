package location

import (
	"github.com/AndroidGoLab/binder/parcel"
)

// Code generated by aidlgen. DO NOT EDIT.

type NanoAppState struct {
	NanoAppId          int64
	NanoAppVersion     int32
	IsEnabled          bool
	NanoAppPermissions []string
}

var _ parcel.Parcelable = (*NanoAppState)(nil)

func (s *NanoAppState) MarshalParcel(
	p *parcel.Parcel,
) error {
	p.WriteInt64(s.NanoAppId)
	p.WriteInt32(s.NanoAppVersion)
	p.WriteBool(s.IsEnabled)
	p.WriteStringList(s.NanoAppPermissions)
	p.WriteInt32(-1) // null ToArray(newNanoAppRpcService[0])
	return nil
}

func (s *NanoAppState) UnmarshalParcel(
	p *parcel.Parcel,
) error {
	var _err error
	s.NanoAppId, _err = p.ReadInt64()
	if _err != nil {
		return _err
	}
	s.NanoAppVersion, _err = p.ReadInt32()
	if _err != nil {
		return _err
	}
	s.IsEnabled, _err = p.ReadBool()
	if _err != nil {
		return _err
	}
	{
		_sl, _slErr := p.ReadStringList()
		if _slErr != nil {
			return _slErr
		}
		s.NanoAppPermissions = _sl
	}
	return nil // opaque ToArray(newNanoAppRpcService[0]): cannot skip without known wire format
}
